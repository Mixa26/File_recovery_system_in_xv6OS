...
sys_lsdel(void)
{
	...
	//char matrix[64][DIRSIZ+1];
	//result = matrix[0];

	//result = (char*)malloc(64 * (DIRSIZ+1));
	...
	for(off = 0; off < ip->size && i < 64; off += sizeof(de))
	{
		...
		if(de.del == '1')
		{
			...
			delDirs++;
			strncpy(result+((DIRSIZ+1)*i), de.name, DIRSIZ+1);
...
sys_rec(void)
{
	...
	if((dp = nameiparentLEGIT(path, name)) == 0)
		return -1;
